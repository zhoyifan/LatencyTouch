...
namespace Valve.VR
{
    ...
    public class SteamVR_Behaviour_Pose : MonoBehaviour
    {
        ...
        public void ChangeLatency(long lat){
            ...
            if(lat==0){
                myHistoryQueue.Clear();
            }else{
                if(milliseconds>lat&&myHistoryQueue.Count>0){
                    myHistroyStep before=myHistoryQueue.Peek();
                    while(myHistoryQueue.Count>0&&(System.DateTime.Now.Ticks-before.timeInTicks)>(lat*TimeSpan.TicksPerMillisecond)){
                        myHistoryQueue.Dequeue();
                    }
                }
            }
